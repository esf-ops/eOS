...
function networkFailureMessage(joinErr: string, attemptedUrl: string) {
  ...
  return `Backend is not reachable at ${base}. Attempted ${attemptedUrl}. (${joinErr}) Confirm backend-core is running and this origin is allowed by CORS.`;
}
...
export async function apiFetch(path: string, options: ApiFetchOptions) {
  ...
  if (res.status === 403) throw new ApiError("You do not have access to this head.", 403, json ?? text);
