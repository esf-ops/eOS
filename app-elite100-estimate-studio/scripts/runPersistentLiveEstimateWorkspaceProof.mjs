/**
 * Local-only screenshots for persistent live Estimate Workspace (not committed).
 *   node app-elite100-estimate-studio/scripts/runPersistentLiveEstimateWorkspaceProof.mjs --start-servers
 */
...
const outDir = join(root, ".local/review/persistent-live-estimate-workspace");
...
try {
  await shot("01-draft-takeoff-live-estimate", studio("draft"), async (p) => {
    await p.waitForSelector('[data-testid="eq-takeoff-iframe"]');
    await p.waitForSelector('[data-testid="eq-verified-estimate-section"]');
  });
  await shot("02-dimension-edit-draft-total", studio("r2"), async (p) => {
  ...
  await shot("03-add-line-crane", studio("commercial"), async (p) => {
  ...
  await shot("04-account-adjustment-3pct", studio("commercial"), async (p) => {
  ...
  await shot("05-vanity-program-card", studio("commercial"), async (p) => {
  ...
  await shot("06-island-waterfall-actions", studio("r2"), async (p) => {
    await p.waitForFunction(
      () => window.__takeoffReviewReady?.type === "TAKEOFF_REVIEW_READY",
      null,
      { timeout: 45000 }
    ...
    await frame
      .locator('[data-testid="ctr-island-waterfall-actions"], [data-testid="ctr-add-left-waterfall"]')
      .first()
      .waitFor({ timeout: 20000 })
      .catch(() => {});
  });
  await shot("07-published-r1-editable-r2", studio("revision-history"), async (p) => {
    await p.waitForSelector('[data-testid="eq-revision-history-section"], [data-testid="eq-de-r1-remains-active"]');
  });
  writeFileSync(
    join(outDir, "README.md"),
    `# Persistent live Estimate Workspace screenshots\n\nLocal review only — do not commit.\n`
  );
} finally {
  await browser.close();
  killAll();
