...
/**
 * Sync inbox — explicit Preview then confirm Import.
 * Empty POST body only; never sends mailbox/tenant/token/URL fields.
 */
export default function MailboxSyncModal({
  ...
  async function runPreview() {
    ...
    try {
      ...
      setMessages(Array.isArray(res.messages) ? (res.messages as MailboxPreviewMessage[]) : []);
      setPhase("previewed");
    } catch (e) {
      clearPreviewData();
  ...
  return (
    <div className="eq-modal-backdrop" role="presentation">
      <div className="eq-modal" role="dialog" aria-modal="true" aria-labelledby="eq-mailbox-sync-title">
        <header className="eq-modal-head">
          <div>
            <h2 id="eq-mailbox-sync-title">Sync inbox</h2>
            <p className="eq-muted">
              Read-only preview of {mailboxDisplay || "the server-configured quotes mailbox"}. Mailbox
              address, tenant, and Graph credentials stay on the server. Nothing is imported until you
              confirm.
            </p>
          </div>
          <button
        ...
        <div className="eq-modal-body">
          {phase === "idle" || phase === "error" ? (
            <div className="eq-state">
              <p>
                Click <strong>Preview mailbox</strong> to load recent messages. Classification and
                Takeoff stay off.
              </p>
              {error ? (
            ...
            <>
                    ...
                    return (
                      <li key={m.graphMessageId} className="eq-mailbox-item">
                        <label className="eq-mailbox-row">
                          ...
                          />
                          <div>
                            <div className="eq-cell-primary">{m.subject || "Unknown subject"}</div>
              ...
              <div className="eq-mailbox-confirm">
                <label className="eq-confirm-check">
                  ...
                  />
                  <span>
                    I confirm importing the selected messages into the Estimate Queue (metadata only).
                  </span>
                </label>
                <button
                  type="button"
