...
const OUT_FILE = join(REPO_ROOT, OUTPUT_PATH);
...
console.log(`Wrote ${OUT_FILE} (${ROOM_COUNT} rooms)`);

// ═══════════════════════════════════════════════════════════════════════════
function buildListsSheet() {
  const ws = wb.addWorksheet("_Lists", { state: "veryHidden" });
  let row = 1;
  ...
  row++;
  // Vanity size codes
  const vanitySizeRow = row;
  ws.getCell(row, 1).value = "vanitySize";
  VANITY_PROGRAM_2026.forEach((v, i) => {
    ws.getCell(row, i + 2).value = v.code;
  });
  wb.definedNames.add(`List_PricingMode`, `_Lists!$B$1:$C$1`);
  wb.definedNames.add(`List_MaterialProgram`, `_Lists!$B$2:$C$2`);
  wb.definedNames.add(`List_ProgramOverride`, `_Lists!$B$3:$D$3`);
  wb.definedNames.add(`List_RoomType`, `_Lists!$B$4:$H$4`);
  wb.definedNames.add(`List_YesNo`, `_Lists!$B$5:$C$5`);
  wb.definedNames.add(`List_VanityTier`, `_Lists!$B$6:$D$6`);
  wb.definedNames.add(`List_SinkType`, `_Lists!$B$7:$E$7`);
  wb.definedNames.add(`List_PriceGroup`, `_Lists!$B$${priceGroupRow}:$I$${priceGroupRow}`);
  wb.definedNames.add(`List_VanitySize`, `_Lists!$B$${vanitySizeRow}:$O$${vanitySizeRow}`);
}

function buildConstantsSheet() {
  const ws = wb.addWorksheet("_Constants", { state: "veryHidden" });
  const rows = [
    ["Material use tax %", MATERIAL_USE_TAX_PERCENT / 100],
    ["OOC wholesale %", OOC_WHOLESALE_PERCENT / 100],
    ["OOC direct %", OOC_DIRECT_PERCENT / 100],
  ...
  rows.forEach(([label, val], i) => {
    ...
    ws.getCell(i + 1, 2).value = val;
  });
  wb.definedNames.add("Const_TaxRate", "_Constants!$B$1");
  wb.definedNames.add("Const_OocWholesale", "_Constants!$B$2");
  wb.definedNames.add("Const_OocDirect", "_Constants!$B$3");
  wb.definedNames.add("Const_Tearout", "_Constants!$B$4");
  wb.definedNames.add("Const_VanityThreshold", "_Constants!$B$5");
  wb.definedNames.add("Const_VanityTrip", "_Constants!$B$6");
}
...
function buildRateBookSheet() {
  ...
  wb.definedNames.add("RateBook_Groups", `'${RB}'!$A$4:$A$11`);
  wb.definedNames.add("RateBook_Wholesale", `'${RB}'!$B$4:$B$11`);
  wb.definedNames.add("RateBook_Direct", `'${RB}'!$C$4:$C$11`);
  protectSheet(ws);
}
...
function buildRoomCalculationsSheet() {
  ...
  for (let i = 0; i < ROOM_COUNT; i++) {
    ...
    setFormula(ws, `M${row}`, `=IF(${m}${R.vanityProg}${row}="Yes",0,ROUND(K${row}*Const_TaxRate,2))`, true);
    setFormula(ws, `N${row}`, `=IF(${m}${R.vanityProg}${row}="Yes",0,ROUND(L${row}*Const_TaxRate,2))`, true);
...
function rateFormula(row) {
  const m = `'${MT}'!`;
  return `=IF(${m}${R.vanityProg}${row}="Yes",0,IF(${m}${SETTINGS.pricingMode}="Direct-Retail",INDEX(RateBook_Direct,MATCH(${m}${R.group}${row},RateBook_Groups,0)),INDEX(RateBook_Wholesale,MATCH(${m}${R.group}${row},RateBook_Groups,0))))`;
}

function oocPremiumFormula(row) {
  const m = `'${MT}'!`;
  return `=IF(${m}${R.vanityProg}${row}="Yes",0,IF(C${row}="Out-of-Collection",ROUND(P${row}*IF(${m}${SETTINGS.pricingMode}="Direct-Retail",Const_OocDirect,Const_OocWholesale),2),0))`;
}
...
function buildAddonsSheet() {
  ...
  // Unit price row (hidden reference)
  ADDON_CATALOG.forEach((a, i) => {
    ws.getCell(2, i + 2).value = a.price;
  });
  ...
  for (let i = 0; i < ROOM_COUNT; i++) {
    ...
    totalCell.value = {
      formula: `=SUMPRODUCT(${startCol}${row}:${endCol}${row},${startCol}$2:${endCol}$2)+IF(${m}${R.tearout}${row}="Yes",Const_Tearout,0)`
    };
...
function buildVanityProgramSheet() {
  ...
  for (let i = 0; i < ROOM_COUNT; i++) {
    ...
    setFormula(
      ws,
      `I${row}`,
      `=IF(C${row}<>"Yes",0,IF(F${row}="Kitchen ≥35 sf",INDEX(O$7:O$20,MATCH(D${row},N$7:N$20,0)),IF(F${row}="Kitchen <35 sf",INDEX(P$7:P$20,MATCH(D${row},N$7:N$20,0)),IF($B$4>=Const_VanityThreshold,INDEX(O$7:O$20,MATCH(D${row},N$7:N$20,0)),INDEX(P$7:P$20,MATCH(D${row},N$7:N$20,0)))))`,
      true
    );
    setFormula(
      ws,
      `J${row}`,
      `=IF(C${row}<>"Yes",0,IF(G${row}="Oval White",0,IF(G${row}="Oval Bisque",10,25))*INDEX(Q$7:Q$20,MATCH(D${row},N$7:N$20,0)))`,
      true
    );
    setFormula(ws, `K${row}`, `=IF(C${row}<>"Yes",0,H${row}*Const_VanityTrip)`, true);
...
function buildOutOfCollectionSheet() {
  ...
  for (let i = 0; i < ROOM_COUNT; i++) {
    ...
    setFormula(
      ws,
      `F${row}`,
      `=IF('${RC}'!C${row}="Out-of-Collection",IF('${MT}'!${SETTINGS.pricingMode}="Direct-Retail",Const_OocDirect,Const_OocWholesale),0)`,
      true
    );
...
function buildScenarioTestingSheet() {
  ...
  ws.getCell("B13").value = { formula: "=ROUND(10*45+10*45*Const_TaxRate,2)" };
...
function buildStartHereSheet() {
  ...
  const lines = [
    ...
    "",
    "Regenerate workbook: node tools/quote-math-simulator/buildQuoteMathWorkbook.mjs"
  ];
