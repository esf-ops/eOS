...
export function apiPatch(path: string, token: string, json: unknown) {
  return apiFetch(path, token, { method: "PATCH", body: JSON.stringify(json) });
}

export function apiPost(path: string, token: string, json?: unknown) {
