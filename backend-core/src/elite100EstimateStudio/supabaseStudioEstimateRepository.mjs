...
export class SupabaseStudioEstimateRepository {
   ...
   * approved row. Prior remains the active customer publication target until
   * the newer revision successfully publishes.
   */
  async createSiblingRevisionFrom(organizationId, estimateId, input, actorUserId = null) {
    ...
    if (error) {
      ...
      throw persistenceError("Failed to create measurement revision sibling", error);
    }
    return dbRowToStudioEstimate(data[0]);
  }
