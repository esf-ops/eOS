/**
 * elite100-config-delta-v1 — pure deterministic configuration delta pricing engine.
 *
...
import {
  ...
  ELITE100_CONFIG_DELTA_SCHEMA_VERSION,
...
function priceOptionLine(opt, locked) {
  ...
  if (treatment === "included" || treatment === "no_change") {
    ...
    amountCents = roundLine(unitCents * qty);
  } else if (mode === "per_sf") {
    const milli = sfToMilli(Number(locked.chargeableCounterSfTotal || 0));
    amountCents = roundLine(mulRateCentsByMilliSf(unitCents, milli) * qty);
  } else if (mode === "per_lf") {
    const lfMilli = sfToMilli(Number(locked.edgeLinearFeetTotal || 0));
    amountCents = roundLine(mulRateCentsByMilliSf(unitCents, lfMilli) * qty);
...
export function normalizeTrustedCalculationInput(raw) {
  ...
  if (!raw.engineVersion || raw.engineVersion !== ELITE100_CONFIG_DELTA_ENGINE_ID) {
    throw fail("engine_version_mismatch", `engineVersion must be ${ELITE100_CONFIG_DELTA_ENGINE_ID}`);
  }
...
/**
 * Run elite100-config-delta-v1 calculation.
 * @param {Record<string, unknown>} rawInput
 */
export function calculateElite100ConfigDelta(rawInput) {
  ...
  const preAdjustmentSubtotalCents =
  ...
  const configuredExactCents = preAdjustmentSubtotalCents + spahnCents;
  ...
  const materialGroupDeltas = roomResults.map((r) => ({
    roomKey: r.roomKey,
    fromGroup: r.baselineMaterialGroup,
    toGroup: r.selectedMaterialGroup,
    // Exact option/material deltas are computed vs baseline room economics when provided
    exactMaterialDeltaCents:
      r.baselineMaterialGroup === r.selectedMaterialGroup && !input.baseline?.rooms
        ? 0
        : null
  }));

  // When baseline rooms provided, compute exact material deltas per room
  if (input.baseline?.rooms) {
    for (const r of roomResults) {
      const b = input.baseline.rooms.find((x) => (x.roomKey || x.name) === r.roomKey);
      if (!b) continue;
      const bGroup = normalizeGroupCode(b.materialGroup || b.selectedMaterialGroup || r.baselineMaterialGroup);
      ...
      const delta = r.materialSellCents + r.materialUseTaxCents - (bSell + bTax);
      const slot = materialGroupDeltas.find((d) => d.roomKey === r.roomKey);
      if (slot) slot.exactMaterialDeltaCents = delta;
    }
  }
  ...
  const internal = {
    schemaVersion: ELITE100_CONFIG_DELTA_SCHEMA_VERSION,
    engineVersion: ELITE100_CONFIG_DELTA_ENGINE_ID,
    inputFingerprint,
...
export { ELITE100_CONFIG_DELTA_ENGINE_ID };
