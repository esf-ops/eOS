...

{
  ...
  // Explicit attachment selection must not reuse a different plan's job.
  const JPEG_BYTES = Buffer.from([
    0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10, 0x4a, 0x46, 0x49, 0x46, 0x00, 0x01, 0x01, 0x00, 0x00,
    0x01, 0x00, 0x01, 0x00, 0x00, 0xff, 0xd9
  ]);
  const multi = await repo.createCase({
    organizationId: ORG_A,
    createdByUserId: "user-1",
    status: "qil_received",
    sourceMessage: {
      graphImmutableMessageId: "msg-multi",
    ...
    attachments: [
      {
        ...
        sizeBytes: PDF_BYTES.length,
        safeFilename: "a.pdf",
        sourceAttachmentId: "graph-pdf-1",
        support: "direct_pdf",
      ...
      },
      {
        ...
        safeFilename: "b.jpg",
        sourceAttachmentId: "graph-jpg-1",
        support: "image_needs_review",
  ...
  assert.ok(multi?.id);
  let jpgWorkspace = 0;
  const pdfOpen = await openEstimateForIntakeCase({
    ...common,
    intakeCaseId: multi.id,
    body: { attachmentKey: "graph-pdf-1" },
    fetchAttachmentBytes: async () => PDF_BYTES,
    createWorkspace: async ({ quoteFileId }) => {
      workspaceCalls += 1;
      return { takeoffJobId: `job-${quoteFileId}` };
    }
  });
  const jpgOpen = await openEstimateForIntakeCase({
    repository: repo,
    organizationId: ORG_A,
    intakeCaseId: multi.id,
    actorUserId: "user-1",
    body: {
      attachmentKey: "graph-jpg-1",
      markAsPlan: true,
      manualPlanOverride: true
    },
    repositoryMode: "memory",
    getSupabase: () => ({}),
    fetchAttachmentBytes: async () => JPEG_BYTES,
    ingestFile: async ({ sha256 }) => ({
      quoteFileId: `qf-${sha256.slice(0, 8)}`,
      created: true
    }),
    createWorkspace: async ({ quoteFileId }) => {
      jpgWorkspace += 1;
      return { takeoffJobId: `job-jpg-${quoteFileId}` };
    }
  });
  assert.notEqual(pdfOpen.takeoffJobId, jpgOpen.takeoffJobId);
  assert.equal(jpgOpen.created, true);
  assert.equal(jpgWorkspace, 1);
  console.log("ok: explicit JPG selection does not reuse prior PDF job on same case");
}
