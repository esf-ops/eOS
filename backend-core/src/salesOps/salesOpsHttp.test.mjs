...
async function main() {
  ...
  assert.equal(perfBody.readiness.actualSfAvailable, false);
  assert.equal(perfBody.readiness.attributionActive, false);
  assert.equal((await fetch(`${base}/api/sales-ops/me/operating-view`, authA)).status, 200);
  ...
  assert.equal((await fetch(`${base}/api/sales-ops/team/people`, authB)).status, 404);
  assert.equal((await fetch(`${base}/api/sales-ops/team/people`, authAdmin)).status, 200);
  assert.equal((await fetch(`${base}/api/sales-ops/team/${REP_A}/operating-view`, authAdmin)).status, 200);
  assert.equal((await fetch(`${base}/api/sales-ops/team/${REP_A}/accounts?limit=50`, authAdmin)).status, 200);
  assert.equal((await fetch(`${base}/api/sales-ops/team/${REP_A}/book-intelligence`, authAdmin)).status, 200);
  assert.equal((await fetch(`${base}/api/sales-ops/team/${REP_A}/operating-view`, authB)).status, 404);
