...
**Account Directory vs Finance:** Account Directory is the identity spine and **Account 360** customer context (exact QuickBooks root links, customer A/R, customer invoices/payments/quotes/sales orders, relationship timeline). Finance Head is company accounting truth (`GET /api/finance/*` P&L, Balance Sheet, cash, A/P, GL). Account 360 must not display company-wide Finance facts. See `FEATURE_DECISIONS.md` §318.
