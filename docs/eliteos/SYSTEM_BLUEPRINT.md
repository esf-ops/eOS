...
| **Sales Ops Head (`app-sales-ops`)** | **`https://sales-ops.eliteosfab.com`** (do **not** reuse `sales.eliteosfab.com`, which is the Sales dashboard). Set **`HEAD_URL_SALES_OPS`**. Slug **`sales_ops`**. Staff auth + head access. One persistent salesperson context for manager/admin (sales-role locked to self) across Overview, Accounts, Plan, and Performance. Actual SF is independent of a published Goal and is not blocked by unresolved sibling identity. Primary nav is Overview / Accounts / Plan / Performance / Team; Plan Builder, Identity Review, and Baseline Gap live under Setup. Versioned, admin-managed sales plans with a **guided Plan Builder** and a salesperson-facing plan experience. Monthly targets remain **explicit `sales_ops_plan_period_targets` rows**. Account recommendations are two-dimensional (`sales_account_intelligence_v1`: role + health). Identity gaps show production unavailable, never zero. Monday full-fidelity source mirror (Layer A) plus Sales Ops projection (Layer B), Brain/Vercel Cron READ-ONLY Monday schedules (`LIGHT_ACCOUNT` every 5 minutes, hourly `DEEP_REFRESH`, nightly `FULL_RECONCILE`; writes disabled; webhooks not enabled), and admin Account Identity Review (exact Monday `{boardId}:{itemId}` links; bulk human approval of unique exact-name 1:1 candidates after preview; salesperson and starter-book filters; Sales Ops UI shows governed salesperson names, never raw UUIDs, and groups identity review into High confidence / Manual review / No candidate / Conflict). Org-admin Baseline gap shows a diagnostic/potential reconstruction (may include unapproved candidates) separately from the authoritative activation reconstruction (approved Monday identity plus approved historical mappings only). Writes remain disabled until separately approved. See `FEATURE_DECISIONS.md` §334–§353. |
