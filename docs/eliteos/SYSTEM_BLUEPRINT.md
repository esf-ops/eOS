...
**Account Directory vs Finance:** Account Directory is the identity spine and **Account 360** customer context (exact QuickBooks root links, staff-safe customer A/R, customer quotes/sales orders/invoices/payments including available history from Jan 2025 when the Finance transaction index is governed). Finance Head is company accounting truth (`GET /api/finance/*` P&L, Balance Sheet, cash, A/P, GL). Account 360 must not display company-wide Finance facts, profitability, or owner economics. See `FEATURE_DECISIONS.md` §318–§319. **Status reconciliation (Phase 4–4B):** the Phase 4 classifier remains the authority (`FEATURE_DECISIONS.md` §320). Phase 4B is an ADMIN-only Status Review queue for exceptions only; humans decide one account at a time with durable `status_reconciliation_reviewed` audit dispositions and evidence fingerprints (`FEATURE_DECISIONS.md` §321). No Apply All, no auto-link, no fuzzy Active.
