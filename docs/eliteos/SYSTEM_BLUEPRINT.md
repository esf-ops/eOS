...
| **Sales Ops Head (`app-sales-ops`)** | **`https://sales-ops.eliteosfab.com`** (do **not** reuse `sales.eliteosfab.com`, which is the Sales dashboard). Set **`HEAD_URL_SALES_OPS`**. Slug **`sales_ops`**. Staff auth + head access. Versioned, admin-managed sales plans with a **guided Plan Builder** (person/territory, production goal + milestone path, governed account-book intelligence, activity KPIs, management rhythms, compensation selector) and a salesperson-facing plan experience. Monthly targets remain **explicit `sales_ops_plan_period_targets` rows**. Account recommendations are two-dimensional (`sales_account_intelligence_v1`: role + health from Monday ownership, Account Directory identity, and Moraware completed-install facts; QuickBooks optional/unavailable). Identity gaps show production unavailable, never zero. Scorecards, gated Goal vs Actual, Monday full-fidelity source mirror (Layer A) plus Sales Ops projection (Layer B), Brain/Vercel Cron READ-ONLY Monday schedules (`LIGHT_ACCOUNT` every 5 minutes, hourly `DEEP_REFRESH`, nightly `FULL_RECONCILE`; writes disabled; webhooks not enabled), and admin Account Identity Review (exact Monday `{boardId}:{itemId}` links; bulk human approval of unique exact-name 1:1 candidates after preview; salesperson and starter-book filters; Sales Ops UI shows governed salesperson names, never raw UUIDs, and groups identity review into High confidence / Manual review / No candidate / Conflict). Org-admin Baseline gap reconciles the starter-book May–July completed-install totals without treating current Monday owner as historical truth. Account 360 lazy-loads governed detail, updates, subitems, file metadata, and Docs. Reconcile is batched with durable progress (`sales_ops_monday_sync_state`). Writes remain disabled until separately approved. See `FEATURE_DECISIONS.md` §334–§350. |
...
**Sales QuickBooks Financial Truth sync (ops):** Windows ODBC worker `sync-sales-financials.ps1` remains SELECT-only; production wrapper `run-sales-qb-sync.ps1` loads `C:\eliteOS\config\sales-qb-sync.env`, logs under `C:\eliteOS\logs\sales-qb-sync\`, default incremental 60-day lookback. Optional Task Scheduler task `eliteOS QuickBooks Sales Sync` every 2 hours (installer preview/`-Preflight` unless `-Apply`). Separate from Account Directory `QB_AD_CUSTOMER_*`. **Phase 1 ListID enrichment:** worker SELECTs ODBC `CustomerId` → `qb_customer_list_id`; Brain resolves `qb_root_customer_list_id` from `ad_qb_customer_facts` (exact ParentId walk; never CustomerName). SQL: `eliteos_sales_quickbooks_financial_truth_listid_v2.sql` (manual). **Account Directory Financials (Slice A):** read-only profile tab joins prepared facts only via active `quickbooks_desktop` external link root ListID — see `FEATURE_DECISIONS.md` §309 / §313 / §314. **Slice B:** true A/R aging from Invoices.DueDate + Terms; deterministic collection attention; planned one-time backfill 2025-01-01→current then 60-day incremental — §315.

**QuickBooks Full Finance Foundation (Phase 1):** Isolated ODBC domains (master / revenue_ar / ap / cash / accounting) → `POST /api/internal/finance/quickbooks-sync` → `qb_finance_*` prepared facts. SELECT-only; no QB writes; no 2025 historical backfill in this phase. Accrual is canonical. P&L/BS stored from QuickBooks report views as reconciliation (not summed forms). Opening BS as-of 2024-12-31. Shared CData single-flight lock with Sales. See `FEATURE_DECISIONS.md` §316 and `docs/eliteos/QUICKBOOKS_FINANCE_BACKFILL_READINESS.md`.

**Account Directory QuickBooks customer enrichment (v1 Phases 0–2):** Separate Windows ODBC worker posts prepared customer/job facts to `POST /api/internal/account-directory/quickbooks-customer-sync` (`ad_qb_customer_facts` / `ad_qb_customer_sync_runs` / `ad_qb_link_suggestions`). Account Directory identity tables are not overwritten; confirmed links remain `quickbooks_desktop` external links via existing `link-quickbooks` only (no create-and-link in v1). Jobs stay facts only (`ParentId`/`Sublevel`). Human terminal suggestions (`dismissed`/`linked`) are not reopened by later syncs. Unattended path: `run-ad-qb-customer-sync.ps1` from the eOS working copy on the QB Server (`$PSScriptRoot`; not a second `C:\eliteOS` clone), config `C:\eliteOS\config\ad-qb-customer-sync.env`, logs `C:\eliteOS\logs\account-directory-qb-customer-sync\`, optional nightly task `slabOS Account Directory QB Customer Sync` (installer preview/`-Preflight` only unless `-Apply`; no CLI Windows password). Separate from Sales `QB_SALES_*`. See `FEATURE_DECISIONS.md` §311.

**Account Directory vs Finance:** Account Directory is the identity spine and **Account 360** is the staff-safe **customer operating workspace** (near-full-screen overlay: identity, contacts, locations, internal staff notes and follow-ups on the Account Directory UUID, exact QuickBooks root links, customer A/R, customer quotes/sales orders/invoices/payments, relationship timeline, UUID-linked estimates, and deterministic Insights with evidence). Ordinary Account Directory EDIT cannot PATCH lifecycle `status`; status writes require ADMIN (Status Review or a direct ADMIN PATCH). Account 360 directory audit history is ADMIN/`canViewAudit` only; ordinary VIEW omits `auditHistory`. The directory list uses a wide desktop shell (~1500px) with a compact directory-health strip (Total / Active / Prospects / Needs Review from the base list). Default name/status pages return cheap row fields first; visible-page connections/A/R/YTD/follow-ups/notes/last activity hydrate from a batched `/accounts/list-intelligence` request. `/summary` is not called on Account Directory mount. List YTD is Jan 1→today; Account 360 Moraware Operations remains calendar-2026. Derived sorts (A/R, YTD, follow-up, connections, last activity) rank the full filtered population before pagination. See `FEATURE_DECISIONS.md` §331–§333. Relationship never blanks Account 360; missing history is an empty state, not a crash. Finance Head is company accounting truth (`GET /api/finance/*` P&L, Balance Sheet, cash, A/P, GL). Account 360 must not display company-wide Finance facts, profitability, or owner economics, and must not generate AI financial truth. See `FEATURE_DECISIONS.md` §318–§333. **Moraware identity (Option B):** exact Moraware Account IDs link to Account Directory UUIDs via `account_directory_external_links` (`external_system = moraware`). One Moraware ID has at most one active AD UUID; one AD UUID may have many active Moraware IDs. Shop prefixes such as `Dyersville-` are booking-shop labels, not customer locations. Fuzzy candidates never create links; humans confirm one ID at a time. A **final-action review overlay** can prioritize human-reviewed CONNECT/CREATE rows one-by-one; the plan JSON/CSV is operator-local and is **not** identity authority — runtime re-resolves exact Moraware `source_account_id` and still requires explicit YES (`FEATURE_DECISIONS.md` §330). Do not set `ACCOUNT_DIRECTORY_MORAWARE_FINAL_ACTIONS_PATH` on the production web host. Account 360 exposes a staff-safe `moraware` relationship: linked IDs, 2026 job count, **2026 Job Worksheet Sq.Ft.**, earliest/latest job date, and a bounded recent-jobs list from typed `brain_moraware_jobs` on **`CURRENT_MORAWARE_JOB_SET`** (latest successful complete uncapped full census watermark; incremental overlay only; no `raw_payload` in API responses, no material/color/room/edge in UI yet, no production-stage KPI). **Worksheet scope intelligence (TRUSTED_NOW raw, §327):** canonical grain is Job Worksheet `source_form_id` (Color/SqFt/Room/Edge presence/Thickness/Sink Type + selected raw fabrication fields). Prepared table `sales_moraware_job_worksheet_facts` uses the same FULL-census epoch as job facts; live worksheet population requires the shared `moraware_population` lock (standalone acquire-before-source-load, or outer owner token). Material Mix, normalized backsplash, and upgrade scores remain unimplemented. Query failure uses unavailable/null, never a false zero. Latest complete import group is **not** the current population once incrementals exist (`FEATURE_DECISIONS.md` §326). Canonical `brain_moraware_jobs` writers share exclusive `eos_sync_locks` name `moraware_population` (held across FULL/INCREMENTAL crawl+import+prepared-fact rebuild; import API fail-closed). **Status reconciliation (Phase 4–4B):** the Phase 4 classifier remains the authority (`FEATURE_DECISIONS.md` §320). Phase 4B is an ADMIN-only Status Review queue for exceptions only; humans decide one account at a time with durable `status_reconciliation_reviewed` audit dispositions and evidence fingerprints (`FEATURE_DECISIONS.md` §321). Ordinary EDIT cannot bypass that path via `PATCH /accounts/:id` `status`. No Apply All, no auto-link, no fuzzy Active.
