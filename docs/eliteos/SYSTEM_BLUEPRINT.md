...
**Sales QuickBooks Financial Truth sync (ops):** Windows ODBC worker `sync-sales-financials.ps1` remains SELECT-only; production wrapper `run-sales-qb-sync.ps1` loads `C:\eliteOS\config\sales-qb-sync.env`, logs under `C:\eliteOS\logs\sales-qb-sync\`, default incremental 60-day lookback. Optional Task Scheduler task `eliteOS QuickBooks Sales Sync` every 2 hours (installer preview/`-Preflight` unless `-Apply`). Separate from Account Directory `QB_AD_CUSTOMER_*`. See `FEATURE_DECISIONS.md` §309.
