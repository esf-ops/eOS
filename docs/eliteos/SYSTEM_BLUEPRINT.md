...
| **Sales Ops Head (`app-sales-ops`)** | **`https://sales-ops.eliteosfab.com`** (do **not** reuse `sales.eliteosfab.com`, which is the Sales dashboard). Set **`HEAD_URL_SALES_OPS`**. Slug **`sales_ops`**. Staff auth + head access. Versioned, admin-managed sales plans with **explicit monthly period targets**, Brain-persisted scorecards, gated Goal vs Actual performance (candidate metric `COMPLETED_INSTALLATION_SF` from JobTracker view 219 typed form facts; attribution remains gated on Account Directory identity — unavailable, not zero), Monday full-fidelity source mirror (Layer A) plus Sales Ops projection (Layer B), Brain/Vercel Cron READ-ONLY Monday schedules (`LIGHT_ACCOUNT` every 5 minutes, hourly `DEEP_REFRESH`, nightly `FULL_RECONCILE`; writes disabled; webhooks not enabled), and admin Account Identity Review (exact Monday `{boardId}:{itemId}` links; bulk human approval of unique exact-name 1:1 candidates after preview; salesperson and starter-book filters). Account 360 lazy-loads governed detail, updates, subitems, file metadata, and Docs. Reconcile is batched with durable progress (`sales_ops_monday_sync_state`). Writes remain disabled until separately approved. See `FEATURE_DECISIONS.md` §334–§345. |
