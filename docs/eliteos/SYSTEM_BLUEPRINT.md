...
- **Launcher deployment URLs:** each head’s **Open** target comes from **`HEAD_URL_*`** env vars on **backend-core** (`backend-core/src/me/headDeploymentUrls.js` → `GET /api/me/heads`). **Unset** keys produce **empty** URLs — cards must **not** advertise localhost on production. **`app-home`** production bundles **do not** infer `http://localhost:*` when the API omits a URL (local Vite defaults apply **only in dev**, see `app-home/src/lib/config.ts`).
