...
**Account Directory vs Finance:** Account Directory is the identity spine and **Account 360** is the staff-safe **customer operating workspace** (near-full-screen overlay: identity, contacts, locations, exact QuickBooks root links, customer A/R, customer quotes/sales orders/invoices/payments, relationship timeline, UUID-linked estimates, and deterministic Insights with evidence). Finance Head is company accounting truth (`GET /api/finance/*` P&L, Balance Sheet, cash, A/P, GL). Account 360 must not display company-wide Finance facts, profitability, or owner economics, and must not generate AI financial truth. See `FEATURE_DECISIONS.md` §318–§322. **Status reconciliation (Phase 4–4B):** the Phase 4 classifier remains the authority (`FEATURE_DECISIONS.md` §320). Phase 4B is an ADMIN-only Status Review queue for exceptions only; humans decide one account at a time with durable `status_reconciliation_reviewed` audit dispositions and evidence fingerprints (`FEATURE_DECISIONS.md` §321). No Apply All, no auto-link, no fuzzy Active.
