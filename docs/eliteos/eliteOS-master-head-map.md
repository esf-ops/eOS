...
- **Account Directory Head** (`app-account-directory/`, slug **`account_directory`**) — identity spine for customers/accounts (canonical name, contacts, locations, aliases, exact QuickBooks links) plus **Account 360**, the staff-safe customer operating workspace (near-full-screen overlay: maintainable contacts/locations, internal staff notes and follow-ups on the Account Directory UUID, customer A/R, customer quotes/sales orders/invoices/payments, relationship timeline, UUID-linked estimates, deterministic Insights with evidence). Directory audit history is ADMIN/`canViewAudit` only. Lifecycle `status` PATCH is ADMIN-only; ordinary EDIT cannot bypass Status Review. Not a CRM replacement and **not** company Finance (P&L / Balance Sheet / cash / A/P / GL / profitability). Exact identity only — never fuzzy name joins. Status semantics: QuickBooks is authority for established customer vs inactive; Account Directory remains lifecycle authority. Phase 4 classifier is the status-reconciliation authority; Phase 4B is an ADMIN-only one-account Status Review queue for exceptions. Exact Moraware Account IDs attach to the same UUID via governed external links (Option B: many Moraware IDs per AD UUID; each Moraware ID at most one AD UUID) in an admin Moraware Links queue (`FEATURE_DECISIONS.md` §318–§329).
- Sales Performance Head
- **Sales Ops Head** (`app-sales-ops/`, slug **`sales_ops`**, **`https://sales-ops.eliteosfab.com`**, Brain `/api/sales-ops/*`) — versioned sales-plan lifecycle (Plan Admin for authorized users), scorecards with historical target snapshots, and a read-only Monday Account Master List full-fidelity mirror (Account Directory remains canonical identity) with lazy Account 360 (detail, updates, subitems, file metadata, Docs). Distinct from the Sales dashboard (`sales` / `https://sales.eliteosfab.com`). Normal reps see only their own published/effective plan and currently assigned accounts. See `FEATURE_DECISIONS.md` §334–§339.
...
- QuickBooks / Accounting Integration Head — connector/import/sync-health surfaces (future; distinct from Intelligence Head).  
