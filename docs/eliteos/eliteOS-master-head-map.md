...
- **Sales Ops Head** (`app-sales-ops/`, slug **`sales_ops`**, **`https://sales-ops.eliteosfab.com`**, Brain `/api/sales-ops/*`) — versioned sales-plan lifecycle (Plan Admin for authorized users), scorecards with historical target snapshots, and a read-only Monday Account Master List full-fidelity mirror (Account Directory remains canonical identity). Distinct from the Sales dashboard (`sales` / `https://sales.eliteosfab.com`). Normal reps see only their own published/effective plan and currently assigned accounts. See `FEATURE_DECISIONS.md` §334–§337.
