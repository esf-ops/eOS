...
| **Incremental** | Strategy: **`creation_window_plus_rolling_exact_refresh`** (2026-08-17). Moraware XML exposes **no** authoritative modified-since / changed-since list filter — **creationDate is not modification detection**. Creation-window discovery uses **`collectCompleteIncrementalJobList`** (Foundation `buildJobQueryByProcessInnerXml` pagination) — **not** `collectGlobalSyncStyleJobListSample` (that helper’s default `maxPages=50`×`pageSize=100` produced the live-read **5000-row sample ceiling**). Complete list traversal is required (`COMPLETE_LIST_DISCOVERY`); partial/capped traversal fails closed as **`CREATION_DISCOVERY_INCOMPLETE`** (no exact refresh, no cursor advance). List ordering by creationDate is **not** assumed. Candidate set = creation-window ∪ explicit ∪ rolling CURRENT. Exact refresh hard-capped at **100** deduplicated candidates. Bootstrap when `advanced_to` is null: **FULL census `started_at` − overlap**. Dry-run `--allow-moraware-read` sets `MORAWARE_DISCOVERY_QUIET_LOGS=1`. Mutation adapters absent. Overlay keeps parent FULL epoch A. Absence never deletes membership/worksheet rows. Cursor in `organization_integration_configs` (`moraware_incremental_cursor`). FULL census remains required for global absence / new epoch. View 222 is not identity authority. |
