...
| **Material** | Exact v4 Direct/Retail and Wholesale tables per group (Promo/A–F/Remnant) as specified — no Public Quote 25% markup. **v4 Wholesale Remnant is $45/SF**, intentionally isolated from legacy `quoteCalculator.js`/`studioEstimatePricing.mjs` ($50/SF, unchanged for v1–3). 2% material use tax on stone only (countertop/backsplash/side-splash/waterfall-leg material) — never sinks, faucets, products, or any labor. Measured vs. billed SF returned separately. |
...
| **Mapping boundary** | A conservative server mapper applies only choices that have existing Studio V2 draft authority: catalog-backed material pricing group/color when it maps safely, and supported edge profiles on existing rooms/pieces. Product selections without a first-class Studio V2 editable product field and all physical-scope requests (openings, waterfalls, splash changes, dimensions implied by notes, extra rooms/pieces) are persisted as **not automatically applied** estimator-review metadata. Customer-supplied totals, formulas, rates, dimensions, approval, publication, acceptance, and lifecycle fields are never mapped. |
