...
|-------|--------|
| **Date** | 2026-07-19 |
| **Decision** | While AI Takeoff is queued/processing/failed/disabled, the consolidated Takeoff worksheet remains **editable** with Add Room / Add Piece / **Remove room** / **Remove piece** / Save draft. Empty rooms render immediately. **Remove** is distinct from Include/exclude: removals hard-delete from the working draft and persist `reviewState.deletedRoomIds` / `deletedRunIds` tombstones so AI merge/polling cannot resurrect them. **Save & merge** keeps the local estimator draft, merges AI-only findings via `saveMergeTakeoffDrafts` / `mergeAiDraftPreservingConfirmed` (precedence: deletion → estimator-owned → AI append), and must **not** force-reload a raw AI-only result. Polling soft-merges and never displaces owned geometry. Manual geometry is estimator-owned (`_estimatorOwned` / reviewState manual ids + `_meta.estimatorConfirmed`). Server **`selectAuthoritativeTakeoffResult`** prefers approved → estimator-confirmed (including geometry markers / correction `result_summary` fallback) over newer raw AI rows. Queue status **Takeoff draft ready** requires usable geometry (pieces **or** roomCount). Stale intake `relationship_status=queued` is ignored when the takeoff job is completed/failed/processing. Hosted AI completion uses **waitUntil best-effort** plus durable cron **`GET\|POST /api/internal/takeoff/process-queued`**. |
| **Why** | Estimators needed durable room/piece removal; Save & merge had force-reloaded AI and dropped manual rooms; Add Room was invisible for empty rooms; queue links stuck at queued. |
