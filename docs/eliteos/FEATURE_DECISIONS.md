...
| **Decision** | One estimator revision contains measurements + adjustments. Visible action is **Edit Estimate** (calls existing `open-measurement-revision`). Commercial save uses **PATCH** `/api/elite100-estimate-studio/estimates/:id` (`updateScope`) + calculate — never obsolete `POST …/scope`. Parent aggregates save status; section stays dirty and shows “Your estimate adjustments were not saved. Try again.” on failure. Section renamed **Estimate Adjustments** with Additional charges/credits, Account adjustment, Vanity Program, Waterfalls. No Crane $350 preset. Vanity bowl count derives from openings; package is a governed select. Island Takeoff offers Add left/right waterfall. |
| **Why** | Production 404 on commercial save; competing measurement/commercial revision UX; false Saved+Failed; implementation-oriented commercial UI. |
| **SQL** | None. |
