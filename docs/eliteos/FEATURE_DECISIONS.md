...
| **Incremental** | True Moraware incremental crawl is **not** automated yet. When incrementals exist, worksheet refresh keeps the same FULL epoch; authoritative per-job `forms[]` may reconcile obsolete forms for that job only; absence from an incremental batch must never delete that job's worksheet facts (removal planner exists; pipeline wiring of per-job delete remains future work). |
