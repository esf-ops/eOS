...
| **Hero cards** | **YTD Jobs** / **YTD Sq Ft** = org-wide governed CURRENT-set YTD jobs (includes unlinked Moraware jobs). **Customers with YTD Activity** = distinct canonical AD UUIDs with ≥1 exact Moraware-linked YTD job (unlinked jobs do not invent a customer). **YTD Estimate Win Rate** = Internal Estimate sold/won ÷ (sold/won + lost) for the current calendar year through today (`FEATURE_DECISIONS.md` §332). Open/undecided estimates are excluded from the denominator. Unavailable (including missing lost coverage) → `—`, never a fake zero. Open A/R remains on the account table and Account 360 Financials, not the landing-page hero. Hero totals are **not** summed from table rows. |
...
| **Sorting** | Header sorts the **entire filtered population**, then paginates. DB-native sorts (name, status, updated) paginate first and enrich only the visible page. Derived sorts (YTD, follow-up attention, connections, A/R, last activity) still enrich the full filtered set before pagination. Safety bounds fail closed: YTD job load, account full-population sort, and note/follow-up heads use cap+1 detection and never present a truncated set as complete. Exact active-link loaders page to completion and are checked against `countActiveExternalLinks` when a complete population is required. |
...
|-------|--------|
| **Date** | 2026-08-18 |
| **Decision** | Default Account Directory list reads (name / status / updated) filter and sort the base account population, paginate, then batch-enrich **only the visible page** (exact QB/Moraware links, A/R, YTD activity, follow-up summary, note counts, contact/email/location, last activity). Whole-organization job/note/follow-up/A/R intelligence is not required to render page 1. Derived sorts still rank the full filtered population before pagination and keep cap+1 fail-closed behavior. |
| **Hero** | Four cards: YTD Jobs, YTD Sq Ft, Customers with YTD Activity, **YTD Estimate Win Rate**. Open A/R is not a landing-page hero KPI. The account table loads independently of `/summary`; hero skeletons/`—` may remain while summary finishes. |
