...
### 344. Mac mini hourly incremental and nightly scheduled pipeline are separate LaunchAgents (2026-08-28)

| Field | Value |
|-------|--------|
| **Date** | 2026-08-28 |
| **Decision** | Production Mac mini paths are `/Users/chrishenely/eOS-worker` and `/Users/chrishenely/.eliteos/moraware-worker.env`, logs under `~/Library/Logs/eliteOS`. **Hourly** `com.eliteos.moraware-incremental` continues to run only `npm run eos:moraware:incremental` via `run-moraware-incremental.sh` and must not be modified. **Nightly** `com.eliteos.moraware-nightly` is a separate LaunchAgent (`StartCalendarInterval` 1:30 America/Chicago) that execs `run-moraware-nightly-macos.sh` → governed `run-moraware-worker.sh` → `runScheduledMorawarePipeline` including View 219. No crontab on this host. Cloud/VM wrappers keep `/opt/eliteos/eOS` and `/etc/eliteos` defaults; `LOG_DIR` is overridable. Both writers use the same distributed lock `eos_sync_locks.lock_name = moraware_population` (4h lease, 15m heartbeat, stale when `expires_at` elapsed, no steal of a healthy lock). Nightly waits up to 45 minutes for that lock rather than overlapping or skipping immediately. View 219 is not on the hourly job. |
| **Why** | Hourly incremental is healthy and must stay intact. View 219 needs the scheduled rebuild path once per day. Overlap must be lock-serialized, not assumed away by clock skew. |
...
| **Account intelligence** | Two independent dimensions — **role** (Anchor / Growth opportunity / Reactivation / New-unproven) and **health** (Healthy / Watch / Needs attention / Data gap) — under governed ruleset **`sales_account_intelligence_v1`**. See §348. |
...
| **Out of scope** | Publishing a real salesperson plan, QuickBooks financial classification, Monday/Moraware/QuickBooks writes, fabricating compensation rules. |
| **Impacted** | `backend-core/src/salesOps/`, `app-sales-ops/`, this doc, SYSTEM_BLUEPRINT. |

### 348. Sales Ops account intelligence role/health v1 (2026-08-31)

| Field | Value |
|-------|--------|
| **Date** | 2026-08-31 |
| **Decision** | Plan-authoring account intelligence is two independent dimensions. **Role** is strategic type: `ANCHOR`, `GROWTH_OPPORTUNITY`, `REACTIVATION`, `NEW_UNPROVEN`. **Health** is attention: `HEALTHY`, `WATCH`, `NEEDS_ATTENTION`, `DATA_GAP`. An Anchor may be Needs Attention. Overdue CRM contact is a health signal and does not remove Anchor. Identity gap is health `DATA_GAP` with no production-derived role; unavailable production is never zero. Plan priority checkboxes and plan-only role/health overrides store on `accountExpectations.planBook` and do not mutate source accounts, attribution facts, or commission eligibility. |
| **Ruleset** | Named governed v1: **`sales_account_intelligence_v1`**. `lookback_months=6`, `anchor_producing_months=5`, `comparison_window_days=90`, `growth_pct=15`, `watch_decline_pct=15`, `attention_decline_pct=25`, `reactivation_days=120`, plus `growth_min_prior_sf=100` as a noise floor so tiny bases cannot become Growth from percentage movement alone. Authority is `backend-core/src/salesOps/salesOpsAccountIntelligence.mjs`; the frontend must not hardcode thresholds. Every book-intelligence response returns the ruleset version. |
| **Role** | Anchor: completed-install production in ≥5 of last 6 calendar months. Growth: trailing comparable 90-day SF ≥15% above the prior window, with sufficient history and prior SF ≥ noise floor. Reactivation: historical producer whose last completed-install evidence is ≥120 days before as-of. New/unproven: identity exists but history is insufficient for the other roles. |
| **Health** | Needs attention: trailing comparable decline ≥25%, or reactivation dormancy. Watch: 15–24% decline, next-contact overdue, or stale last contact (≥ comparison window). Healthy: no current governed attention/watch signal. Data gap: no approved Account Directory identity. Missing next strategic milestone is shown as a reason and does not by itself change health. |
