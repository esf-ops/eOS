...
| **eliteOS Internal Estimate Head** | `app-internal-estimate/` | Staff quoting, Direct/Wholesale, room builder, Elite Program color catalog, custom lines, print | **Yes** | **`/api/internal-quotes/*`** (calculate, save, list, **`GET /material-colors`**, get, patch, duplicate) | `https://internal.eliteosfab.com` (or `estimate.*`) | **Testing** | **Target UX / phases:** [`INTERNAL_ESTIMATE_PRODUCT_SPEC.md`](../quote-platform/INTERNAL_ESTIMATE_PRODUCT_SPEC.md). Gaps: in-place vs revision save, branch `ESF-` numbering, full visual editor, server PDF, file upload; Monday internal columns may need env mapping. |
