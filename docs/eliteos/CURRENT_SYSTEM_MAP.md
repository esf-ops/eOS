...
**Source material:** [`SYSTEM_BLUEPRINT.md`](./SYSTEM_BLUEPRINT.md), [`FEATURE_DECISIONS.md`](./FEATURE_DECISIONS.md), [`INTERNAL_ESTIMATE_PRODUCT_SPEC.md`](../quote-platform/INTERNAL_ESTIMATE_PRODUCT_SPEC.md) (target Internal Estimate UX and phases), [`eliteOS-master-head-map.md`](./eliteOS-master-head-map.md), [`domain-routing-plan.md`](./domain-routing-plan.md), repo `app-*` and `backend-core` route mounts, `.cursor/rules/*.mdc` (architecture, security, quote-platform).
...
|------------------------------|------------|---------|------|----------------------------------------|-------------------------------|----------------|-------------|
| **eliteOS Home / Launcher** | `app-home/` | Sign-in, launcher cards from **`GET /api/me`** and **`GET /api/me/heads`**, deep links to other heads | **Yes** | `/api/me`, `/api/me/heads` | `https://www.eliteosfab.com` | **Working** | Staff heads under **`*.eliteosfab.com`** share **cookie-backed Supabase session** (`shared/eliteos-supabase/`); Public Quote stays per-origin (see §5). |
...
| **eliteOS Internal Estimate Head** | `app-internal-estimate/` | Staff quoting, Direct/Wholesale **rate-book math** (live preview + Calculate), guided + room-by-room scope; **sticky scoped-material rollup matches customer Quoted Material Breakdown** (mixed piece-level tiers per room), Elite Program color catalog, structured custom lines (tear-out preset), hybrid section UX; **Visual Layout Canvas v1** (drag/rotate verification boards — non-authoritative, frontend-only layout state); print worksheet with **all-tier** staff comparison and **customer-selected** tier comparison (`internal_ui.customer_estimate_display_groups`); **Phase 2** durable saves (`save_mode`), ESF `quote_number_base` + `-R{n}` revisions | **Yes** | **`/api/internal-quotes/*`** (calculate, save, list, **`GET /material-colors`**, get, metadata **PATCH** — **no snapshot**, duplicate) | `https://internal.eliteosfab.com` (or `estimate.*`) | **Testing** | **Target UX / phases:** [`INTERNAL_ESTIMATE_PRODUCT_SPEC.md`](../quote-platform/INTERNAL_ESTIMATE_PRODUCT_SPEC.md). **Apply migration:** `backend-core/supabase/eliteos_internal_quote_phase2.sql` before relying on ESF/revisions. Gaps: Canvas Phase B (snap/scale/callouts), **server PDF customer estimate reusing display-group selection**, durable **file upload / attachments** (future); Monday internal columns may need env mapping. |
...
| **eliteOS System Admin Head** | `app-system-admin/` | **People & access** (roster, roles, head access, dealer/pricing assignments), **invites**, **user lifecycle** (resend invite, password reset, deactivate/reactivate, guarded delete), **audit/activity visibility** (auth/session events + action logs), **diagnostics** (schema health, legacy quote pipeline / legacy pricing tools). Sales mapping + identity resolution remain embedded. | **Yes** (`admin` or `super_admin`) | **`/api/system-admin/*`** (alias **`/api/admin/*`** for same user routes), schema health, reference, **`/audit-events`** | `https://system.eliteosfab.com` (intended; confirm DNS) | **Working / testing** | Apply `backend-core/supabase/eliteos_audit_foundation.sql` for full org-aware audit columns; Quote Library + Pricing Admin absorb primary quote/pricing UX. |
| **eliteOS Sales Head** | `app-sales/` | Sales performance and intelligence UI | **Yes** | **`/api/sales/*`** (requires auth + `sales` head + allowed roles) | `https://sales.eliteosfab.com` (intended; set `HEAD_URL_SALES`) | **Scaffolded / deployed preview** (depends on deploy) | Launcher availability is env-driven; account/branch attribution remains preview until approved mappings exist and dashboard parity is complete. |
| **eliteOS Sales Ops Head** | `app-sales-ops/` | Versioned admin-managed sales plans, Brain-persisted scorecards, Monday full-fidelity account mirror + operational projection | **Yes** | **`/api/sales-ops/*`** (auth + `sales_ops` head). Webhook `POST /api/integrations/monday/sales-ops/webhook` (challenge live; production board subscription not created — `MONDAY_APP_SIGNING_SECRET` missing) | `https://sales-ops.eliteosfab.com` (`HEAD_URL_SALES_OPS`; DNS-only CNAME to Vercel project target) | **Production read-only** — custom domain live; v1+v2 SQL applied; Monday → eliteOS census run; batched reconcile + durable status; writes disabled. | Normal reps see only their published/effective plan and Monday-assigned accounts. Writes remain disabled. See `FEATURE_DECISIONS.md` §334–§338 and `monday-sales-ops.md`. |
...
| Session / launcher | `GET /api/me`, `GET /api/me/heads`, `POST /api/auth/log-login`, `POST /api/auth/log-event` | No | **Yes** (Bearer JWT) | `/api/me` records throttled `session_seen`; `/api/me/heads` records `tool_access_loaded`; `log-login` records `launcher_opened`; `log-event` supports best-effort `sign_out`. `/api/me/heads` builds launcher cards from profile + `user_head_access` / defaults (see §5). |
...
| **Internal quotes** | **`POST /api/internal-quotes/calculate`**, **`/save`**, **`GET /api/internal-quotes`**, **`GET /api/internal-quotes/material-colors`**, **`GET/PATCH /api/internal-quotes/:id`**, **`POST .../duplicate`** | No | **Yes** + **`requireHeadAccess("quote")`** (stack in `internalQuotesApi.js`) | Staff internal estimate head. |
...
| **System / user admin** | **`/api/system-admin/*`** and **`/api/admin/*`** for **same** advanced user router: users, invite, profile, head-access, dealer-access, pricing-group, **`POST .../send-password-reset`**, **`POST .../resend-invite`**, **`PATCH .../deactivate`**, **`PATCH .../reactivate`**, **`DELETE .../:id`** (guarded), role, reference, schema-health, **`GET .../audit-events`** | No | **Yes** + **`system_admin` head** + **`admin` or `super_admin`** role | Per `systemAdminUserManagement.js`. Service role **only** on the server; audit logs are not exposed to non-admin users. |
...
| **Sales Ops head** | `/api/sales-ops/me*`, `/api/sales-ops/accounts/:id` (+ lazy `/subitems|/updates|/files|/docs|/activity`), `/api/sales-ops/admin/plans*`, `/api/sales-ops/team*`, `/api/integrations/monday/sales-ops/webhook` | Webhook: public (JWT if secret set); **do not create production webhooks in this phase** | **Yes** + `sales_ops` head (except webhook) | Plans are eliteOS-owned lifecycle; Monday is CRM source, not canonical identity. See §334–§338. |
...
- **Supabase Auth** issues JWTs; **staff heads** (`app-home`, `app-system-admin`, `app-internal-estimate`, `app-quote-library`, `app-pricing-admin`) persist the browser session via **`shared/eliteos-supabase/`** — chunked cookies with **`Domain=.eliteosfab.com`** on HTTPS hosts so **Home → other staff subdomains** does **not** require a second login. **`app-quote` (Public Quote)** continues using **default per-origin storage** and stays **public**.
...
- **Cross-subdomain staff UX:** Cookie-backed Supabase storage (`SYSTEM_BLUEPRINT.md` §3) replaces the prior **per-subdomain localStorage** isolation for assigned staff heads. Logout in any participating head clears shared cookies.
- **Heads not yet on shared cookies:** `app-quote` (public), `app-sales`, `app-executive`, `app-brain-health`, etc. — unchanged unless wired to the same adapter later.
...
| **`eos_login_log`**, **`eos_action_log`** (`auth_schema.sql` + `eliteos_audit_foundation.sql`) | **Yes** expected; apply audit foundation for full columns | **Yes** | Auth/session events + meaningful action audit for System Admin visibility | Exact Supabase password submit is not visible to Brain; first authenticated API/session event is used as sign-in/seen signal. |
...
- **Product behavior (Phase 1 workspace):** hybrid section nav (non-wizard); **Job Info** block (account/Direct, contacts, customer, elite job name, project address, branch, salesperson, entered-by); **Room / Area** default **guided shape** with preset-first shapes (incl. backsplash piece + waterfall math); **Manual sq ft** secondary; **rapid linear** hidden in this head and in room builder when opened from here; **tear-out** as structured **custom line** preset (legacy `addOns.tearout` stays **0**); materials with **Color TBD** toggle + optional catalog default; **sticky estimator summary** with scope sf, material summary, readiness, Calculate / Save / Print / Library; save snapshot adds **`internal_ui.job_info`** when client sends `job_info` (`internalQuotesApi` merge only).
- **Gaps:** Save from hydrated `quoteId` still creates a **new** header row (honest UI copy); verify **internal Monday** board + **`MONDAY_INTERNAL_*`** column mapping; **user testing** before calling production-ready; customer estimate PDF / server PDF / full visual editor remain future phases per product spec.
...
**Head URL env vars (Brain):** `HEAD_URL_PUBLIC_QUOTE`, `HEAD_URL_INTERNAL_ESTIMATE` (alias `HEAD_URL_QUOTE`), **`HEAD_URL_QUOTE_LIBRARY`**, `HEAD_URL_PRICING_ADMIN`, `HEAD_URL_EXECUTIVE`, `HEAD_URL_BRAIN_HEALTH`, `HEAD_URL_SYSTEM_ADMIN`, `HEAD_URL_SALES`, **`HEAD_URL_SALES_OPS`** (`headDeploymentUrls.js`).
