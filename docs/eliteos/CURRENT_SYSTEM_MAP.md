...
| **eliteOS Internal Estimate Head** | `app-internal-estimate/` | Staff quoting, Direct/Wholesale, room builder, Elite Program color catalog, custom lines, print | **Yes** | **`/api/internal-quotes/*`** (calculate, save, list, **`GET /material-colors`**, get, patch, duplicate) | `https://internal.eliteosfab.com` (or `estimate.*`) | **Testing** | In-place revision save vs new row; internal Monday column mapping verification. |
