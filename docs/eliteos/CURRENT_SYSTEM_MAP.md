...
| **Sales Ops head** | `/api/sales-ops/me*`, `/api/sales-ops/accounts/:id` (+ lazy `/subitems|/updates|/files|/docs|/activity`), `/api/sales-ops/admin/plans*`, `/api/sales-ops/team*`, `/api/integrations/monday/sales-ops/webhook` | Webhook: public (JWT if secret set); **do not create production webhooks in this phase** | **Yes** + `sales_ops` head (except webhook) | Plans are eliteOS-owned lifecycle; Monday is CRM source, not canonical identity. See §334–§336. |
