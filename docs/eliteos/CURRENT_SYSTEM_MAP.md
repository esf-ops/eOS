...
| **eliteOS System Admin Head** | `app-system-admin/` | Users, invites, head access, dealer/pricing assignments, embedded admin tools (sales mapping, identity, quote pipeline, legacy quote admin UIs) | **Yes** (`admin` or `super_admin`) | **`/api/system-admin/*`** (alias **`/api/admin/*`** for same user routes), schema health, reference | `https://system.eliteosfab.com` (intended; confirm DNS) | **Working / testing** | Invite email UX, end-to-end new-user head visibility, docs/manual tests lag code. |
...
**Embedded in System Admin (same `app-system-admin` bundle):** Sales Account Mapping, Identity Resolution readiness, Quote Pipeline panel, Quote Pricing admin view—these call **`/api/admin/*`** route families (not duplicated as separate deployables).
...
| **System / user admin** | **`/api/system-admin/*`** and **`/api/admin/*`** for **same** advanced user router: users, invite, profile, head-access, dealer-access, pricing-group, password reset, role, reference, schema-health | No | **Yes** + **`system_admin` head** + **`admin` or `super_admin`** role | Per `systemAdminUserManagement.js`. |
...
- **User roster, detail, profile updates, head access, dealer access, pricing group assignment, password reset, role changes, invites** via **`/api/system-admin/*`** (and **`/api/admin/*`** aliases).
