  ...
  try {
    ...
    if (auth.authMode === "dev_bypass") {
      return NextResponse.json({
        ...
        items: [],
        ambiguous: false,
        warning: "Account search requires Brain auth — not available in auth bypass mode.",
      });
    }
    const res = await brainFetch(`/api/slab-ai/accounts/search?q=${encodeURIComponent(q)}&limit=10`, auth);
